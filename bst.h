...
protected:
    ...
    int helpBalance(Node<Key, Value>* node, bool& isBalanced) const;
...
void BinarySearchTree<Key, Value>::remove(const Key& key)
{
    ...
    if (child != NULL) {
        ...
        if (rnode == root_) {
            ...
            rnode->getParent()->setLeft(child);
        } else {
            rnode->getParent()->setRight(child);
        }
        child->setParent(rnode->getParent());
    } else {
       
        if (rnode == root_) {
            root_ = NULL;
        } else if (rnode->getParent()->getLeft() == rnode) {
            rnode->getParent()->setLeft(NULL);
        } else {
            rnode->getParent()->setRight(NULL);
        }
    }
...
//helper function to clear recursively
template<typename Key, typename Value>
...
BinarySearchTree<Key, Value>::getSmallestNode() const
{
    ...
    while (cur != NULL && cur->getLeft() != NULL) {
        cur = cur->getLeft();
    }
    return cur;
}
...
bool BinarySearchTree<Key, Value>::isBalanced() const
{
    // TODO
    bool bal = true;
    int x = helpBalance(root_,bal);
    return bal;
}

//helper function to check balance factors recursively
template<typename Key, typename Value>
int BinarySearchTree<Key, Value>::helpBalance(Node<Key, Value>* node, bool& isBalanced) const {
    if (node == nullptr) {
        return -1;
    }

    int lefth = helpBalance(node->getLeft(), isBalanced);
    int righth = helpBalance(node->getRight(), isBalanced);
